...
def verify_password():
    ...
    tk.Label(pw_window, text="請輸入管理密碼：", font=("標楷體", 12)).pack(pady=10)
    pw_entry = tk.Entry(pw_window, show="*", font=("標楷體", 12))
    ...
    tk.Button(pw_window, text="確認", command=check, font=("標楷體", 12)).pack(pady=10)
...
def submit_report():
    ...
    try:
        ...
        if not anydesk or anydesk == "未填寫":
            messagebox.showerror("錯誤", "Anydesk號碼為必填欄位")
            return
