...
def download_excel_from_nas(filename):
    if not current_config:
        return None
    ftp = FTP()
    ftp.connect(current_config["ftp_host"], current_config["ftp_port"])
    ftp.login(current_config["ftp_user"], current_config["ftp_pass"])
    ftp.cwd(current_config["ftp_folder"])
...
贊耀資訊"""
    ...
    try:
        with smtplib.SMTP(
            COMMON_CONFIG["smtp_server"],
            COMMON_CONFIG["smtp_port"],
            local_hostname="localhost",
        ) as server:
            server.starttls()
            server.login(COMMON_CONFIG["sender_email"], COMMON_CONFIG["smtp_password"])
...
def engineer_ui_nas():
    ...
    def search_report():
        ...
        if not current_config:
            messagebox.showerror("錯誤", "請先選擇公司")
            return
        ...
        report_id = f"{current_config['prefix']}{date_part}-{num_part.zfill(3)}"

        try:
            df_main = download_excel_from_nas(current_config["main_excel"])
        except Exception as e:
    ...
    def submit_solution():
        ...
        try:
            ...
            if not index.empty:
                ...
                upload_excel_to_nas(df_main, current_config["main_excel"])
